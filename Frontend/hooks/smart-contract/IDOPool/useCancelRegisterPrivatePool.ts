...
export const useCancelRegisterPrivatePool = ({
  chainId,
  // <---! CONTRACT PROPS IN ABI STARTS HERE !---> //
  poolAddress,
  // <---! CONTRACT PROPS IN ABI STARTS HERE !---> //
  ...
  const CONTRACT_LABEL = "Cancel Register Private Pool";
  const FUNCTION_EXECUTION_NAME = "onCancelRegisterPrivatePool";
  ...
  const {
    ...
    // <---! PARAMS IN ABI !---> //
    // <---! FUNCTION IN ABI !---> //
    functionName: "cancelRegisterPrivatePool",
    // <---! FUNCTION IN ABI !---> //
  ...
  const onCancelRegisterPrivatePool = async () => {
